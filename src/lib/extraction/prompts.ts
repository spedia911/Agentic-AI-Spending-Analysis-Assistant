...
const STRICT_JSON_CONTRACT = [
  ...
  '      "evidence_region": {"x": 0.0, "y": 0.0, "width": 0.0, "height": 0.0} | null',
  '    }',
  '  ],',
  '  "asset_snapshots": [',
  '    {',
...
export function buildCreditCardExtractionPrompt(source: Pick<ExtractionSourceImage, 'sourceDocumentId' | 'fileName'>): string {
  return [
    ...
    'When possible, include evidence_region as normalized screenshot coordinates from 0 to 1 around the row. Use null if coordinates are not reliable.',
...
export function buildBankActivityExtractionPrompt(source: Pick<ExtractionSourceImage, 'sourceDocumentId' | 'fileName'>): string {
  return [
    ...
    'When possible, include evidence_region as normalized screenshot coordinates from 0 to 1 around the activity row or balance area. Use null if coordinates are not reliable.',
    'Source document ID: ' + source.sourceDocumentId,
    'File name: ' + source.fileName,
    STRICT_JSON_CONTRACT,
  ].join('\n\n');
...
export function buildMixedScreenshotExtractionPrompt(source: Pick<ExtractionSourceImage, 'sourceDocumentId' | 'fileName'>): string {
  return [
    ...
    'When possible, include evidence_region as normalized screenshot coordinates from 0 to 1 around each row or balance area. Use null if coordinates are not reliable.',
    'Source document ID: ' + source.sourceDocumentId,
    'File name: ' + source.fileName,
    STRICT_JSON_CONTRACT,
  ].join('\n\n');
